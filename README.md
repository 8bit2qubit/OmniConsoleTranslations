...
- The `en-US/Resources.resw` in each project is the **English baseline** — use it as your reference; don't translate it.
...
5. Add your language to [`index.json`](index.json) (see below).
...
A `.resw` file is standard XML. You only edit the text inside `<value>`:
...
- **`{0}` `{1}` etc. are parameter placeholders** — keep them; their position can move to fit your language, but don't change or remove them.
- **`AUTHENTICITY.md` filename**: a few strings (e.g., the certificate-details text) reference the `AUTHENTICITY.md` file on the main repo. Only **Traditional Chinese (`zh-TW`)** and **Simplified Chinese (`zh-CN`)** have a localized `AUTHENTICITY.zh-TW.md`; **all other languages should reference the English `AUTHENTICITY.md`**. So keep `AUTHENTICITY.md` as-is in your translation (unless you're translating into Chinese, where it's `AUTHENTICITY.zh-TW.md`).
- **`untranslated:new` / `untranslated:changed` comments** mark entries that still need translating:
...
- Only the **`<value>`** and the `untranslated:*` comment matter. Don't add other comments or restructure the file.

---

## ✍️ index.json — language list & translator credits

`index.json` lists which languages are available for each version. Translator names live **only here** (not inside the `.resw` files), and OmniConsole shows them as the credit next to your language:
...
- `translators` — your name(s); shown in-app as the translator credit. Multiple translators: `["name1", "name2"]`.
- `revision` — bump this by 1 when you publish a fix to an existing translation, so installed copies know to refresh. New languages start at `1`.

When adding a new language, add a new entry to the latest version's array. When fixing one, bump its `revision`.

---

## ✅ Before you open a pull request

- The two folders use your exact language code (e.g., `es-ES`, not `es` or `ES-es`).
- `{OmniConsole}` placeholders and `{0}`/`{1}` parameters are intact.
- Translated entries have their `untranslated:*` comments removed.
- Your language is listed in `index.json` with your translator name.
