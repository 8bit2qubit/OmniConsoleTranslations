...
5. 把你的語言加進 [`index.json`](index.json)（見下方）。
6. 發出 **Pull Request**。
...
- **`untranslated:new` / `untranslated:changed` 註解**標記仍待翻譯的項目：
...
`index.json` 列出每個版本有哪些語言可用。譯者名稱**只記在這裡**（不放在 `.resw` 檔內），OmniConsole 會把它顯示為你語言旁的署名：
...
- `translators`：你的名字，會在 App 內顯示為譯者署名。多位譯者：`["名字1", "名字2"]`。
- `revision`：當你為既有翻譯發布修正時將它加 1，已安裝的副本便會知道要更新。新語言從 `1` 開始。

新增語言時，在最新版本的陣列加一筆；修正翻譯時，把它的 `revision` 加 1。

---

## ✅ 發出 Pull Request 前

- 兩個資料夾都使用你正確的語言碼（如 `es-ES`，而非 `es` 或 `ES-es`）。
- `{OmniConsole}` 佔位符與 `{0}`／`{1}` 參數都完好無缺。
- 已翻譯的項目都已移除 `untranslated:*` 註解。
- 你的語言已列在 `index.json` 中並附上譯者名稱。
